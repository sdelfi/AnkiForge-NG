...
class TestCreateDeck:
    ...
    def test_empty_name_raises(self, mock_mw: MagicMock) -> None:
        with patch("ankiforge.anki_bridge.deck_manager._get_mw", return_value=mock_mw):
            with pytest.raises(ValueError, match="Имя колоды не может быть пустым"):
                create_deck("")
...
class TestSaveMedia:
    ...
    def test_empty_data_raises(self, mock_mw: MagicMock) -> None:
        with patch("ankiforge.anki_bridge.deck_manager._get_mw", return_value=mock_mw):
            with pytest.raises(ValueError, match="Данные файла не могут быть пустыми"):
...
class TestAddNote:
    ...
    def test_unknown_note_type_raises(self, mock_mw: MagicMock) -> None:
        ...
        with patch("ankiforge.anki_bridge.deck_manager._get_mw", return_value=mock_mw):
            with pytest.raises(ValueError, match="Note type .* не найден"):
    ...
    def test_sets_deck_id_on_note(self, mock_mw: MagicMock) -> None:
        ...
        # Проверяем что note.note_type['did'] установлен
        note = mock_mw.col.new_note.return_value
